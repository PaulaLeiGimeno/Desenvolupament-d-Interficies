...
const App = () => {
  const [dadesSalut, canviaDadesSalut] = useState(null);
  const [imc, canviaIMC] = useState(null);

  const rebreIMC = (unIMC) => {
    canviaIMC(unIMC);
  };
  const rebreDadesSalut = (dades) => {
    canviaDadesSalut(dades);
  };
    ...
    <Provider>
      <View>
        ...
        <View style={{ justifyContent: "center" }}>
          <Titol />
          <Calculadora
            comunicaDadesSalut={rebreDadesSalut}
            comunicaIMC={rebreIMC}
          />
          {dadesSalut !== null && <Resultat dadesS={dadesSalut} />}
        </View>
      </View>
    </Provider>
  );
