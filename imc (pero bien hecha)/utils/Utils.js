...
/**
 * Realitza el càlcul del pes ideal a partir d'un pes i una alçada
 * @function

 * @param {float} unaAlsada L'alçada ien format correcte
 * @returns {Object} Torna un objecte amb el pes mínim i el màxim 
 *
 */
export const calculaPesIdeal = (unaAlsada) => {
  const min = 18.5 * unaAlsada * unaAlsada;
  const max = 24.9 * unaAlsada * unaAlsada;
  return ({ minim: min, maxim: max });
};
 ...
 * @param {float} unPesMin El pes mínim recomanat en format correcte
 * @param {float} unPesMax El pes màxim recomanat en format correcte
 * @returns {String} Indica a l’usuari si ha de guanyar, perdre o mantindre el pes per a situar se dins del rang saludable. 
 */
export const calculaKgAPerdre = (unPes, unPesMin, unPesMax) => {

  if (unPes < unPesMax && unPes > unPesMin) {
    return { msg: "Has de mantindre aquest pes", color: "green" }
  }
  if (unPes < unPesMin) {
    let pesACanviar = unPesMin - unPes;
    return { msg: `Has de guanyar ${pesACanviar.toFixed()} kg.`, color: "orange" }
  }
  if (unPes > unPesMax) {
    let pesACanviar = unPes - unPesMax;
    return { msg: `Has de perdre ${pesACanviar.toFixed()} kg.`, color: "orange" }
  }
};

export const calculaCaloriesDiariesRecomanades = (unPes, unaAlsada, unaEdat, unSexe, unNivellDactivitat) => {
  let factorActivitat = 0;
  switch (unNivellDactivitat) {
    case 'Sedentari':
      factorActivitat = 1.2
      break;

    case 'Lleu':
      factorActivitat = 1.375
      break;
    case 'Moderat':
      factorActivitat = 1.55
      break;
    case 'Alt':
      factorActivitat = 1.725
      break;
    case 'Molt Alt':
      factorActivitat = 1.9
      break;
  ...
  if (unSexe == 'home') {
    return ((10 * unPes + 6.25 * (unaAlsada * 100) - 5 * unaEdat + 5) * factorActivitat)
  } else if (unSexe == 'dona') {
    return ((10 * unPes + 6.25 * (unaAlsada * 100) - 5 * unaEdat - 161) * factorActivitat)
  }
