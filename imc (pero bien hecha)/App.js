...
const App = () => {
  ...
  const [pesIdealRebut, canviaPesIdealRebut] = useState(null)
  const [pesRebut, canviaPesRebut] = useState(null)

  const rebreIMC = (unIMC) => {
    canviaImcRebut(unIMC);
  };

  const rebrePesIdeal = (pesIdeal) => {
    canviaPesIdealRebut(pesIdeal)
  }
  const rebrePes = (pes) => {
    canviaPesRebut(pes)
  }
  ...
  return (
    <View style={{ padding: 20, margin: 20 }} >
      ...
      <View style={{ justifyContent: "center" }}>
        <Titol />
        <Calculadora comunicaIMC={rebreIMC} comunicaPesIdeal={rebrePesIdeal} comunicaPes={rebrePes} />
        <ScrollView>
          {imcRebut !== null && <Resultat imc={imcRebut} ideal={pesIdealRebut} pes={pesRebut} />}
        </ScrollView>
      </View>
    </View >
  );
